...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.n_features * self.embed_size, hidden_size)))
    ...
    def embedding_lookup(self, w):
        ...
        batch_size = list(w.size())[0]
        wFlatten = w.view(batch_size * self.n_features)
        embed = torch.index_select(self.embeddings, 0, wFlatten)
        x = embed.view(batch_size, self.n_features*self.embed_size)
    ...
    def forward(self, w):
        ...
        hidden_layer = F.relu(torch.matmul(self.embedding_lookup(w), self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        dropped_out = self.dropout(hidden_layer)
