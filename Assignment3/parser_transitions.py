...
def minibatch_parse(sentences, model, batch_size):
    ...
    index = list(range(len(unfinished_parses)))

    while len(index) > 0:
        
        batch = []
        for i in index[0:batch_size]:
            batch.append(unfinished_parses[i])
        transitions = model.predict(batch)
        for i in range(len(transitions)):
            unfinished_parses[index[i]].parse_step(transitions[i])
        
        for i in index[0:batch_size]:
            if len(unfinished_parses[i].buffer) == 0:
